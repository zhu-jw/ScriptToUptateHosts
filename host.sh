...
usage()
{	echo $(date +%D\ %H:%M:%S)>>/var/log/hosts/hosts.log
	cat	<<EOL>>/var/log/hosts/hosts.log
提示:
...
get_hosts()
{
		...
		if [ "$QUIET_MODE" = "on" ]; then
				echo "$(date +%D\ %H:%M:%S)|正在更新 hosts...">>/var/log/hosts/hosts.log
				${downer} -s
		else
				echo "$(date +%D\ %H:%M:%S)|正在更新 hosts...">>/var/log/hosts/hosts.log
				${downer}
		...
		if [ $? -ne 0 ]; then
				echo "$(date +%D\ %H:%M:%S)|hosts 下载失败" >>/var/log/hosts/hosts.log
				exit 2
		fi
...
backup_hosts()
{
		echo "$(date +%D\ %H:%M:%S)|bachup hosts">>/var/log/hosts/hosts.log
...
update_hosts()
{
		...
		if [ ! -z "$RANGE" ]; then
				...
				if grep -q "$BEGIN_MARK" "$HOSTS"; then
						...
						if [ $? -eq 0 ]; then
							echo "$(date +%D\ %H:%M:%S)|delete old hosts success," \
							"date marked will be written to new hosts file" \
							>>/var/log/hosts/hosts.log
						else
							echo "$(date +%D\ %H:%M:%S)|delete old hosts filed" \
							>>/var/log/hosts/hosts.log
						fi
				else
						echo "警告: $HOSTS 中没有发现此脚本所作标记，" \
								"更新后会清空之前的所有内容。" >>/var/log/hosts/hosts.log
						echo "请从 $BACKUP_FILE 中手动恢复需要的 hosts 记录" >>/var/log/hosts/hosts.log
				fi
		fi
		...
		sudo cp -f "$swp" "$HOSTS"
		if [ $? -eq 0 ]; then
			echo "$(date +%D\ %H:%M:%S)|update success~~~^_^" >>/var/log/hosts/hosts.log
		else
			echo "$(date +%D\ %H:%M:%S)|update error-_-">>/var/log/hosts/hosts.log
		fi
